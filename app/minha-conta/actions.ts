...
// Retorna a solicitacao de reembolso existente da conta (ou null se nunca pediu).
export async function getRefundRequest(): Promise<RefundRequest | null> {
  const supabase = await createClient()
  const user = await getCurrentUser()
  if (!user) return null
  ...
  if (error) {
    console.error('[v0] Erro ao buscar reembolso:', error.message)
    return null
  }
  return (data as RefundRequest) || null
}

// Solicita o reembolso de TODOS os pedidos pagos da conta. Idempotente: se ja
// existir uma solicitacao, apenas a retorna (nao cria duplicado). O reembolso e
// registrado como "processing" e um informe permanente fica salvo na conta.
export async function requestRefund(): Promise<
  | { success: true; refund: RefundRequest; alreadyRequested: boolean }
  | { error: string }
> {
  const supabase = await createClient()
  const user = await getCurrentUser()
  ...
  // Ja solicitado? Devolve o registro existente sem recriar.
  const existing = await getRefundRequest()
  if (existing) {
    return { success: true, refund: existing, alreadyRequested: true }
  }

  // Consolida os pedidos pagos da conta (convites pagos vinculados ao e-mail).
  ...
  if (email) {
    const { data: invites } = await supabase
      ...
      .eq('email', email)
      .eq('status', 'paid')
    ordersCount = invites?.length || 0
    totalAmount = (invites || []).reduce((sum, i) => sum + Number(i.amount || 0), 0)
  }
  ...
  if (error) {
    console.error('[v0] Erro ao solicitar reembolso:', error.message)
    return { error: error.message }
  }

  revalidatePath('/minha-conta')
  return { success: true, refund: data as RefundRequest, alreadyRequested: false }
}
